    ...
    <div className={`min-h-screen transition-all duration-700 font-sans ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-32 lg:py-48 flex flex-col items-center">
        ...
        <header className="mb-24 text-center space-y-8 animate-float">
          ...
          <h1 className="text-5xl lg:text-8xl font-black tracking-tighter leading-[0.9] drop-shadow-2xl lowercase">
             {t.title}
          </h1>
          ...
          <button 
            className="premium-button mt-4 px-10 py-4 h-14 min-w-[200px] shadow-2xl"
          >
            {t.getStarted}
          </button>
        </header>

        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 w-full mb-32">
          <CounterHook />
          <EffectHook />
          <RefHook />
          <MemoHook />
          ...
          <div 
            ...
            className={`glass-card p-10 flex flex-col items-center justify-center gap-6 border-dashed border-2 transition-all cursor-pointer ${theme === 'dark' ? 'border-slate-800/50 hover:bg-white/5' : 'border-slate-300 hover:bg-slate-200'}`}
          >
